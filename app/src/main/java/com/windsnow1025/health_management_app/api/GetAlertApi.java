...
public class GetAlertApi extends AsyncTask<String, Void, String> {

    private static final String API_URL_SYNC_GET_ALERT = "https://www.windsnow1025.com/learn/api/android/sync/get/alert";
    ...
    private String callApi(String phoneNumber) {
        ...
        try {
            URL url = new URL(API_URL_SYNC_GET_ALERT);
    ...
    // 用于获取Alert信息
    public List<Alert> getAlertInformation(String phoneNumber) {
        ...
        try {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    alert.setAlert_date(jsonResponse.getString("alert_date"));
                    alert.setAlert_cycle(jsonResponse.getString("alert_cycle"));
                    alert.setIs_medicine(jsonResponse.getString("is_medicine"));
                ...
                e.printStackTrace();
                throw new RuntimeException("GetAlertApi 解析用户信息时发生异常: " + e.getMessage());
            }
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("GetAlertApi 获取用户信息时发生异常: " + e.getMessage());
        }
    }
    ...
    protected void onPostExecute(String result) {
        // 在UI线程中处理返回的结果
        if (result != null) {
            Log.d("API Response", "GetAlertApi Raw Response: " + result);
        } else {
            Log.e("API Response", "GetAlertApi 获取手机号时发生异常或为空");
        }
    }
}
