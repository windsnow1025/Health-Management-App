...
public class AlertFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        listView.setOnItemClickListener((parent, view12, position, id) -> {
            ...
            if (isMedicine) {
                transaction.replace(R.id.fragment_container, new MedicineFragment(isMedicine, bind_id, adapter, alertList, alertList.get(position).getId(), is_report, true));
            } else {
                transaction.replace(R.id.fragment_container, new DiagnoseFragment(isMedicine, bind_id, adapter, alertList, alertList.get(position).getId(), is_report, true));
            }
